...
export async function createServer(): Promise<Server> {
  ...
  server.setRequestHandler(ListPromptsRequestSchema, async () => {
    ...
    prompts.push({
      ...
      arguments: [
        {
          name: 'userQuery',
          description: 'Your question or content submission for the assistant.',
          required: true
        }
      ]
    });
  ...
  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    if (request.params.name === 'brand-assistant-persona') {
      const userQuery = request.params.arguments?.userQuery || 'Hello, I have a question.'; // Default query
      
      let personaInstructions = 'You are a helpful assistant. Please be concise.'; // Basic fallback
      const instructionsPath = path.join(__dirname, 'assistant_persona_instructions.md');
      const templatePath = path.join(__dirname, 'assistant_persona_template.md');

      try {
        personaInstructions = await fs.readFile(instructionsPath, 'utf-8');
      } catch (error) {
        // If instructions file doesn't exist or fails to read, try template
        try {
          personaInstructions = await fs.readFile(templatePath, 'utf-8');
          console.log('Loaded persona from template. Detailed instructions file not found.');
        } catch (templateError) {
          console.error('Failed to load both persona instructions and template. Using basic fallback. Error:', templateError);
          // Basic fallback is already set
        }
      }
