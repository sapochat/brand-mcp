...
import { ComplianceEvaluation, ComplianceIssue, IssueType, IssueSeverity } from '../../entities/ComplianceEvaluation.js';
...
export class BrandComplianceServiceImpl implements BrandComplianceService {
  ...
  checkCompliance(content: Content, brand: Brand, context = 'general'): ComplianceEvaluation {
    ...
    const terminologyIssues = this.checkTerminologyCompliance(content, effectiveGuidelines, context);
  ...
  private checkTerminologyCompliance(content: Content, brand: Brand, context: string): ComplianceIssue[] {
    const issues: ComplianceIssue[] = [];
    const text = content.normalizedText.toLowerCase();
    ...
    for (const rule of brand.terminologyGuidelines.terms) {
      if (rule.alternatives) {
        ...
        for (const alternative of rule.alternatives) {
          if (text.includes(alternative.toLowerCase()) && 
      ...
      if (rule.term && rule.avoidInContexts?.includes(context) && 
  ...
  private generateComplianceSummary(score: number, issues: ComplianceIssue[], brandName: string): string {
    if (score >= 80) {
