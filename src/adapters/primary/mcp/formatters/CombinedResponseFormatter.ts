...
} from '../../../../domain/entities/SafetyEvaluation.js';
...
export class CombinedResponseFormatter {
  ...
  private formatComplianceSection(evaluation: ComplianceEvaluation): string {
  ...
  private formatSafetySection(evaluation: SafetyEvaluation): string {
    ...
    const categoriesWithIssues =
      evaluation.categoryEvaluations?.filter(
        (item: CategoryEvaluation) => item.riskLevel !== 'NONE' && item.riskLevel !== 'LOW'
      ) || [];
